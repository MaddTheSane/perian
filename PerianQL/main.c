...
#define PLUGIN_ID "F70477B1-E3D2-49B2-BE01-402B6A4B47F5"
...
static PQLGeneratorplug  *AllocQuickLookGeneratorPluginType(CFUUIDRef inFactoryID);
static void                         DeallocQuickLookGeneratorPluginType(PQLGeneratorplug *thisInstance);
...
//
PQLGeneratorplug *AllocQuickLookGeneratorPluginType(CFUUIDRef inFactoryID)
{
	PQLGeneratorplug *theNewInstance;
	
	theNewInstance = (PQLGeneratorplug *)malloc(sizeof(PQLGeneratorplug));
	memset(theNewInstance,0,sizeof(PQLGeneratorplug));
...
//
void DeallocQuickLookGeneratorPluginType(PQLGeneratorplug *thisInstance)
{
...
HRESULT QuickLookGeneratorQueryInterface(void *thisInstance, REFIID iid, LPVOID *ppv)
{
	...
	if (CFEqual(interfaceID, kQLGeneratorCallbacksInterfaceID)) {
		 ...
		 * return good status.
		 */
		((QLGeneratorInterfaceStruct *)((PQLGeneratorplug *)thisInstance)->conduitInterface)->GenerateThumbnailForURL = GenerateThumbnailForURL;
		((QLGeneratorInterfaceStruct *)((PQLGeneratorplug *)thisInstance)->conduitInterface)->CancelThumbnailGeneration = CancelThumbnailGeneration;
		((QLGeneratorInterfaceStruct *)((PQLGeneratorplug *)thisInstance)->conduitInterface)->GeneratePreviewForURL = GeneratePreviewForURL;
		((QLGeneratorInterfaceStruct *)((PQLGeneratorplug *)thisInstance)->conduitInterface)->CancelPreviewGeneration = CancelPreviewGeneration;
		((QLGeneratorInterfaceStruct *)((PQLGeneratorplug*)thisInstance)->conduitInterface)->AddRef(thisInstance);
		*ppv = thisInstance;
...
{
    return ++((PQLGeneratorplug *)thisInstance)->refCount;
}
...
{
	((PQLGeneratorplug*)thisInstance)->refCount -= 1;
	if (((PQLGeneratorplug*)thisInstance)->refCount == 0){
		DeallocQuickLookGeneratorPluginType((PQLGeneratorplug*)thisInstance );
		return 0;
	} else {
		return ((PQLGeneratorplug*) thisInstance )->refCount;
	}
